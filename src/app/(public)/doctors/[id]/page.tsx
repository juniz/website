...
export default async function DoctorDetailPage({ params }: DoctorDetailPageProps) {
  ...
  const availabilityConfig = {
    today:       { variant: 'success' as const, label: 'Tersedia hari ini' },
    tomorrow:    { variant: 'warning' as const, label: 'Besok tersedia' },
    unavailable: { variant: 'danger' as const,  label: 'Tidak tersedia' },
  };
  ...
  const doctorSchedules = todaySchedules.filter((s) => String(s.doctorId) === String(doctor.id));
  ...
  return (
    <>
      ...
      >
        {/* Doctor availability + experience strip */}
