...
import { AlertCircle, CheckCircle2, Loader2, QrCode, ArrowRight, Home } from 'lucide-react';
import Link from 'next/link';

const STATUS = { PENDING: 'pending', SCANNED: 'scanned', TRANSFERRED: 'transferred', EXPIRED: 'expired', ERROR: 'error' };
...
export default function QrDisplay({ token }) {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      setStatus(res.success ? res.data.status : STATUS.ERROR);
      setLoading(false);
    };
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 20px 60px rgba(4,44,83,0.1)', border: '1px solid var(--color-neutral-200)' }}>
      ...
      {/* Dark header */}
      {/* <div style={{ background: 'var(--color-primary-900)', padding: '2rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, opacity: 0.06 }}>
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs><pattern id="qr-grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="white" strokeWidth="0.5" /></pattern></defs>
            <rect width="100%" height="100%" fill="url(#qr-grid)" />
          </svg>
        </div>
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(55,138,221,0.2)', marginBottom: '1rem' }}>
            <QrCode size={26} style={{ color: 'var(--color-primary-300)' }} />
          </div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-figtree)', color: '#fff', letterSpacing: '-0.02em', marginBottom: '0.375rem' }}>QR Code Pre-Registrasi</h2>
          <p style={{ color: 'var(--color-primary-200)', fontSize: '0.875rem' }}>Tunjukkan QR Code ini ke petugas loket</p>
        </div>
      </div> */}
