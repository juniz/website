...
export default function SchedulePageClient({ initialSchedules = [] }) {
  ...
  const visibleSchedule = useMemo(() => {
    // We can filter by day logic if we wanted, but DB fetches current date.
    if (selectedDay !== 0) return [];
    if (activeFilter === 'all') return initialSchedules;
    return initialSchedules.filter((s) => s.specializationCode === activeFilter);
  }, [selectedDay, activeFilter, initialSchedules]);
            ...
            {days.map((day) => {
              ...
              return (
                <button
                  ...
                  aria-label={`${day.label} ${day.date}${day.isToday ? ' (hari ini)' : ''}`}
                  style={{
                    display: 'flex', flexDirection: 'column', alignItems: 'center',
                    padding: '0.5rem 0.875rem', borderRadius: '8px', minWidth: '56px',
                    border: isSelected ? '1.5px solid var(--color-primary-400)' : '1.5px solid var(--color-neutral-200)',
                    background: isSelected ? 'var(--color-primary-50)' : '#fff',
                    cursor: 'pointer', transition: 'all 150ms ease-out', gap: '0.25rem',
                  }}
                >
                  <span style={{ fontSize: '0.6875rem', color: isSelected ? 'var(--color-primary-600)' : 'var(--color-neutral-600)', fontWeight: 500 }}>
                    {day.isToday ? 'Hari Ini' : day.label}
                  </span>
                  <span style={{ fontSize: '1.0625rem', fontWeight: 700, color: isSelected ? 'var(--color-primary-800)' : 'var(--color-neutral-900)', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
                    {day.date}
                  </span>
                </button>
              );
      ...
      <section className="section-py" style={{ background: 'var(--color-neutral-50)' }}>
        <div className="container-site">
          ...
          <div style={{ marginBottom: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
            <h2 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--color-neutral-900)', fontFamily: 'var(--font-figtree, Figtree, system-ui, sans-serif)' }}>
              {selectedDay === 0 ? todayFull : days[selectedDay]?.label + ', ' + days[selectedDay]?.date}
            </h2>
