...
export default function FacilitiesPageClient({ facilities = [] }) {
  const [activeCategory, setActiveCategory] = useState('Semua');
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFacility, setSelectedFacility] = useState(null);

  // Extract all categories dynamically
  const categories = ['Semua', ...new Set(facilities.map(f => f.category))];
  ...
  return (
    <>
      {/* Banner */}
      <section className="bg-primary-900 text-white py-16 px-4" style={{
        ...
        <div className="container-site" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p style={{
            ...
            Kami menyediakan berbagai fasilitas medis modern dan layanan kesehatan berkualitas tinggi untuk kenyamanan dan pemulihan pasien.
          </p>
        </div>
      </section>
